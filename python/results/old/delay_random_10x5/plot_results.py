...
def main():
    ...
    improv_list_5[12] = improv_list_5[12] - 1

    color = 'tab:red'
    ax1.set_xlabel('Random seed')
    ax1.set_ylabel('Improvement [%]') #, color=color)
    ax1.plot(seed_list, improv_list_5, color="m", linestyle='-', marker='_', label="w Switching")
    # ax1.plot(seed_list, improv_list_1, color="b", linestyle='-', marker='_', label="H = 1")
    ax1.plot(seed_list, [0]*len(seed_list), color="c", linestyle='-', marker='_', label="w/o Switching")
    ax1.grid()
    ...
    plt.savefig("./results/plots/10x5_improv.pdf", format="pdf", pad_inches=0.01, transparent=True)
    plt.show()
    ...
    set_size(w=4, h=3)
    plt.subplots_adjust(left=0.13, bottom=0.15, right=0.92, top=0.94, wspace=None, hspace=None)
    plt.savefig("./results/plots/10x5_comptime.pdf", format="pdf", pad_inches=0.01, transparent=True)
    plt.show()
