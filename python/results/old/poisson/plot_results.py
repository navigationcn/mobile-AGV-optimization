...
def main():
    ...
    plt.savefig(dir_path + "/improv.pdf", format="pdf", pad_inches=0.01, transparent=True)
    # plt.show()
    ...
    set_size(w=4, h=3)
    plt.subplots_adjust(left=0.13, bottom=0.15, right=0.92, top=0.94, wspace=None, hspace=None)
    plt.savefig(dir_path + "/comp_time.pdf", format="pdf", pad_inches=0.01, transparent=True)
    plt.show()
