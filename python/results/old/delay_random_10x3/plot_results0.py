...
def main():
    ...
    ax1.grid()
    plt.legend(title="Horizon length",loc="lower center")
    ...
    plt.savefig("./results/plots/10x3_improv.pdf", format="pdf", pad_inches=0.01, transparent=True)
    # plt.show()
    ...
    set_size(w=4, h=3)
    plt.subplots_adjust(left=0.13, bottom=0.15, right=0.92, top=0.94, wspace=None, hspace=None)
    plt.savefig("./results/plots/10x3_comptime.pdf", format="pdf", pad_inches=0.01, transparent=True)
    plt.show()
